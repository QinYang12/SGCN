import sys, os,pdb
sys.path.insert(0, '..')
from lib import models, graph, utils, spherical_vertex
...
train_labels = scipy.io.loadmat('{}/train_labels.mat'.format(project_data_path))['train_labels'][0]
val_labels = scipy.io.loadmat('{}/val_labels.mat'.format(project_data_path))['val_labels']
test_labels = scipy.io.loadmat('{}/test_labels.mat'.format(project_data_path))['test_labels'][0]
...
if True:
    ...
    model_perf.test(models.cgcnn(L, graphs_adjacency, index, z_xyz[-1], file_out, **params), name, params,
                    train_data, train_labels, val_data, val_labels, test_data, test_labels, file_out)
file_out.close()
