...
class model_perf(object):
    ...
    def test(s, model, name, params, train_data, train_labels, val_data, val_labels, test_data, test_labels, file_out):
        ...
        print('train {}'.format(string), file=file_out)
        string, s.test_accuracy[name], s.test_f1[name], s.test_loss[name] = \
                model.evaluate(test_data, test_labels)
        print('test  {}'.format(string), file=file_out)
        s.names.add(name)
